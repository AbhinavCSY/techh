...
const convertAssetsToWebApps = (): WebApp[] => {
  const statuses: Array<WebApp['status']> = ['secure', 'not-secure', 'unreachable', 'redirected', 'error'];
  ...
  const webApps = useMemo(() => {
    ...
    if (sortBy === 'issues') {
      return apps.sort((a, b) => 
        (b.issues.critical + b.issues.high) - (a.issues.critical + a.issues.high)
      );
    } else if (sortBy === 'status') {
      const statusOrder = { 'not-secure': 0, 'error': 1, 'unreachable': 2, 'redirected': 3, 'secure': 4 };
      ...
      <div className="border-b border-gray-200 bg-white">
        <div className="px-6 py-6 max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Web Applications</h1>
              <p className="text-gray-600 mt-1">Total count of similar-looking Web App Groups: {webApps.length}</p>
            </div>
            <div className="text-right">
              <p className="text-sm text-gray-600">Sort Web Apps by Group count</p>
            </div>
          </div>
      ...
      <div className="p-6 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {webApps.map((app) => {
            ...
            const totalIssues = app.issues.critical + app.issues.high + app.issues.medium;
              ...
              >
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-600 truncate mt-1">{app.url}</p>
                  </div>
                  <div className={cn(
                    'flex items-center justify-center w-8 h-8 rounded-full font-bold text-lg',
                    statusInfo.text,
                    app.status === 'secure' ? 'bg-green-100' : 
                    app.status === 'not-secure' ? 'bg-red-100' :
                    app.status === 'unreachable' ? 'bg-gray-100' :
                    app.status === 'redirected' ? 'bg-yellow-100' :
                    'bg-orange-100'
                  )}>
                    {statusInfo.icon}
                  </div>
                ...
                <div className="mb-3 p-2 bg-white bg-opacity-60 rounded">
                  <p className="text-xs font-medium text-gray-700">{app.statusMessage}</p>
                  {app.statusCode && (
                    <p className="text-xs text-gray-600 mt-1">HTTP {app.statusCode}</p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 mb-3">
                  {app.issues.critical > 0 && (
                    <div className="bg-red-100 rounded p-2 text-center">
                      <p className="text-xs font-bold text-red-900">{app.issues.critical}</p>
                      <p className="text-xs text-red-700">Critical</p>
                    </div>
                  )}
                  {app.issues.high > 0 && (
                    <div className="bg-orange-100 rounded p-2 text-center">
                      <p className="text-xs font-bold text-orange-900">{app.issues.high}</p>
                      <p className="text-xs text-orange-700">High</p>
                    </div>
                  )}
                  {app.issues.medium > 0 && (
                    <div className="bg-yellow-100 rounded p-2 text-center">
                      <p className="text-xs font-bold text-yellow-900">{app.issues.medium}</p>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-300 border-opacity-50">
                  <span className="text-xs text-gray-600">
                    Scanned {Math.floor((Date.now() - app.lastScanned.getTime()) / (1000 * 60))}m ago
                  </span>
                  {totalIssues > 0 && (
                    <span className="text-xs font-semibold text-red-700">{totalIssues} issues</span>
                  )}
                </div>
              </div>
            );
