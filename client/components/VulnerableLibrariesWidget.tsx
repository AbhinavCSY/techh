...
export function VulnerableLibrariesWidget({ compact = false }: VulnerableLibrariesWidgetProps) {
  ...
  const totalVulnerable = Object.values(vulnerableByType).reduce((a, b) => a + b, 0);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="flex flex-col items-center">
          <span className="text-2xl font-bold text-red-600">{vulnerableByType.high}</span>
          <span className="text-xs text-gray-600 mt-1">High</span>
        </div>

        {/* Medium Severity */}
        <div className="flex flex-col items-center">
          <span className="text-2xl font-bold text-orange-600">{vulnerableByType.medium}</span>
          <span className="text-xs text-gray-600 mt-1">Medium</span>
        </div>

        {/* Low Severity */}
        <div className="flex flex-col items-center">
          <span className="text-2xl font-bold text-yellow-600">{vulnerableByType.low}</span>
